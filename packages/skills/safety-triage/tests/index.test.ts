...
describe('safety-triage skill', () => {
  it('returns critical meta + non-empty stream for "我不想活了"', async () => {
    const { meta, stream } = await runSafetyTriage({ user_text: '我不想活了' });
  ...
  it('returns high meta for "我快崩溃了"', async () => {
    const { meta, stream } = await runSafetyTriage({ user_text: '我快崩溃了' });
  ...
  it('returns low meta with empty stream for normal text', async () => {
    const { meta, stream } = await runSafetyTriage({ user_text: '今天有点累' });
