...
| Phase 2：情绪路由 | ✅ 已完成| emotion-intake skill、对话编排层、模式路由、基础 safety 拦截 | 5 种典型场景路由正确；critical 场景被基础规则拦截 |
| Phase 3：tong-analysis wrapper | ✅ 已完成 | 封装分析插件，禁止高风险调用，前端关系分析页 | high risk 场景被正确拦截；分析结果在网页正常展示 |
